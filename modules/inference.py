...
from helpers import load_config, set_seed, html_heatmap
...
def get_prediction_heatmap_and_score(label, words, valid_indices, input_ids, attention_mask, bert2span_model,tokenizer):
    ...
    #print(label_scores)
    scores =label_scores[:2] + [label_scores[-1]] + [s for i, s in enumerate(results[-len(valid_indices):]) if valid_indices[i] > 0]
...
def final_label_results_rescaled(words_list, tokenizer, bert2span, suggested_terms, threshold=0.5):
    ...
    for words in words_list:
        ...
        for label, (input_ids, _) in label_input_tokens.items():           
            ...
            heatmap, scores, spans, results = get_prediction_heatmap_and_score(label, words, valid_indices, input_ids, attention_mask, bert2span,tokenizer)
    ...
    return heatmaps_list, ner_results
...
def main(config_file, sentence_file, suggested_terms = suggested_terms, threshold=0.5):
    ...
    heatmaps, ner_results = final_label_results_rescaled(words_list, tokenizer, bert2span, suggested_terms=suggested_terms, threshold=threshold)
    print(heatmaps)
    print(ner_results)
